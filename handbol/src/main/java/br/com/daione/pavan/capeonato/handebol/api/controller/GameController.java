...
public class GameController {
	...
	public Flux<GameResponse> listAll() {
		return this.gameService.findAll()
				.doOnError(x -> this.utilError.internalServerError("Não existe registro gravado em banco. "));
	}

	@GetMapping("/{id}")
	public Mono<GameResponse> findGame(@PathVariable String id) {
		return this.gameService.findGame(id)
				.doOnError(x -> this.utilError.internalServerError("Não é possível encontrar o registro solicitado. "));
	}

	@PostMapping
	public Mono<GameResponse> createGame(@RequestBody GameRequire game) {
		return this.gameService.createGame(game).doOnError(game1 -> {
			this.utilError.internalServerError("Não foi possível criar um novo jogo, tente novamente mais tarde. ");
		});
	}

	@DeleteMapping("{id}")
	public Mono<GameResponse> deleteGame(String id) {
		return this.gameService.delete(id).doOnError(error -> {
			this.utilError.internalServerError("Não foi possível deletar o objeto selecionado.");
		});
	}

	@PatchMapping("/{id}")
	public Mono<GameResponse> updateGame(@PathVariable String id, GameRequire newGame) {
		return this.gameService.update(id, newGame).doOnError(error -> {
			error.printStackTrace();
			this.utilError.internalServerError("Não foi possível atualizar o registro. Tente novamente mais tarde.");
		});
	}
}
